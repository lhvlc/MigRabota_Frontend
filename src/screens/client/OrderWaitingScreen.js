...
import { View, Text, TouchableOpacity, StyleSheet,
  ...
  Alert, Platform, FlatList, Modal } from 'react-native';
...
export default function OrderWaitingScreen({ route, navigation }) {
  ...
  const handleComplete = async (item) => {
    ...
    try { await completeShift(item.id); await load(); }
    catch (e) { console.error(e); }
      ...
      <Modal visible={ratingModal !== null} transparent animationType="slide">
        <View style={S.overlay}>
          <View style={S.modalBox}>
            <Text style={S.modalTitle}>Оценить {ratingModal?.seeker?.name}</Text>
            <Text style={S.modalSub}>Как прошла смена?</Text>
            <View style={S.starsRow}>
              {[1,2,3,4,5].map(star => (
                <TouchableOpacity key={star} onPress={() => setSelectedStars(star)}>
                  <Text style={[S.star, star <= selectedStars && S.starActive]}>⭐</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <View style={S.quickComments}>
              {COMMENTS.map(c => (
                <TouchableOpacity key={c}
                  ...
                  <Text style={[S.quickTxt, comment === c && S.quickTxtOn]}>{c}</Text>
                </TouchableOpacity>
              ))}
            </View>
...
const S = StyleSheet.create({
  ...
  // Время на карточке смены
  shiftTimeRow: { flexDirection:'row', flexWrap:'wrap', gap:6 },
  shiftTimeBadge: { backgroundColor:'#0D1B2A', borderRadius:8,
    paddingHorizontal:8, paddingVertical:4,
    borderWidth:1, borderColor:'#263550' },
  shiftTimeTxt: { color:'#E0E1DD', fontSize:11, fontWeight:'600' },
  shiftDurationBadge: { backgroundColor:'#C9B47F15', borderRadius:8,
    paddingHorizontal:8, paddingVertical:4,
  ...
  quickBtn: { backgroundColor:'#0D1B2A', borderRadius:20,
    paddingHorizontal:12, paddingVertical:7, borderWidth:1, borderColor:'#263550' },
